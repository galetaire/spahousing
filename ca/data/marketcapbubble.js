...
function makeChart(marketgdp) {
    ...
    new Chart(ctx, {
        ...
        options: {
                ...
                tooltip: {
                    callbacks: {
                        label: function (context) {
                            const d = context.raw;
                            return `${d.year} — PIB: ${d.x.toFixed(2)}, Pressió: ${d.y.toFixed(2)}`;
                        }
                    }
                },
                datalabels: {
            ...
            scales: {
                x: {
                    title: { display: true, text: 'Ràtio bens immobles/PIB' },
                    ticks: { callback: (value) => value.toFixed(2) }
                },
                y: {
                    title: { display: true, text: "Pressió en l'habitatge (baròmetre)" },
