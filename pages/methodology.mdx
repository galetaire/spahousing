...
As a rule of thumb, the data is updated quarterly. In any case, the last update can be consulted in the header of each indicator post.
...
Note that the last update reflects when I last checked for new data availability, which won't match with the data in the chart, since it usually lags some time behind the current date.

## Data sources

The production of the charts is based on data from multiple contrasted statistical institutions. All is reproducible and can be verified by consulting the sources in each indicator post. The following is a summary of all used sources:
