...
Note that the publication date reflects when I last checked for new data availability, which won't match with the data in the chart, since it usually lags some time behind the current date.

## Data sources

The production of the charts is based on data from multiple contrasted statistical institutions. The charts are reproducible and can be verified by consulting the sources in each indicator post. The following is a summary of all used sources:
...
This website has been built using [Github](https://github.com/), [Vercel](https://vercel.com) and [Next.js](https://nextjs.org/). A mirrored website is available under the following [Handshake](https://handshake.org/) address -> ``http://spain.galetaire``.
