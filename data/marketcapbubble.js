...
function makeChart(marketgdp) {
    ...
    new Chart(ctx, {
        ...
        options: {
                ...
                tooltip: {
                    callbacks: {
                        label: function (context) {
                            const d = context.raw;
                            return `${d.year} — GDP: ${d.x.toFixed(2)}, Pressure: ${d.y.toFixed(2)}`;
                        }
                    }
                },
                datalabels: {
            ...
            scales: {
                x: {
                    title: { display: true, text: 'Real Estate-to-GDP ratio' },
                    ticks: { callback: (value) => value.toFixed(2) }
                },
                y: {
                    title: { display: true, text: 'Housing pressure (barometer index)' },
